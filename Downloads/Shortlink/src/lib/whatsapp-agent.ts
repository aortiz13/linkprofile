...
import { getAvailableSlots, createBooking, formatBookingConfirmation } from "@/lib/calcom-api";
...
- **scheduling**: Cuando la persona muestra interés en agendar una reunión/videollamada. Incluí "CHECK_AVAILABILITY" en actions para consultar tu agenda. El sistema te va a inyectar las opciones disponibles.
- **booking_confirm**: Cuando tenés los datos necesarios (email, opción elegida) para confirmar la reunión. Incluí "BOOK_MEETING" en actions con los datos recopilados.
...
# AGENDAMIENTO DE REUNIONES (IMPORTANTE)
Tenés la capacidad de agendar videollamadas directamente desde el chat. El flujo es:
1. Cuando la persona muestra interés en hablar, ofrecele agendar una videollamada.
2. Incluí "CHECK_AVAILABILITY" en actions. El sistema va a inyectar las opciones disponibles en tu próximo turno como [SLOTS DISPONIBLES: ...].
3. Presentá las 3 opciones de forma natural: "Mirá, tengo disponibilidad en estos horarios: 1️⃣ ..., 2️⃣ ..., 3️⃣ ... ¿Cuál te queda mejor?"
4. Cuando la persona elija, pedile su email (el teléfono ya lo tenés). Ejemplo: "Perfecto, pasame tu email así te mando la invitación con el link de la videollamada."
5. Cuando tengas el email y la opción elegida, incluí "BOOK_MEETING" en actions con booking_data: { "slot_index": 0, "email": "...", "name": "..." }.
6. El sistema va a crear la reunión automáticamente y te va a pasar la confirmación.
7. Después de confirmar la reunión, avisale: "Antes de la reunión es posible que te llame para hacerte un par de preguntas así aprovechamos mejor el tiempo de la videollamada."

REGLAS DE AGENDAMIENTO:
- NO inventes horarios. SIEMPRE usá CHECK_AVAILABILITY para obtener horarios reales.
- Si no hay slots disponibles, decile que estás bastante ocupado y ofrecé que te escriba en un par de días.
- El email es OBLIGATORIO para crear la reunión. Si no lo tenés, pedilo.
- Podés ofrecer agendar en cualquier momento de la conversación si la persona muestra interés, pero idealmente después de la fase de qualification.
...
Respondé SIEMPRE en formato JSON válido:
{
  ...
  "stage": "greeting|discovery|qualification|value_delivery|link_offer|followup|closing|scheduling|booking_confirm|escalation|inactive",
...
- actions: "SEND_LINK", "ESCALATE", "SEND_AUDIO:trigger_key", "OPT_OUT", "CHECK_AVAILABILITY", "BOOK_MEETING"
- booking_data: null o { "slot_index": 0, "email": "user@email.com", "name": "Nombre" } (solo cuando action es BOOK_MEETING)
...
// ─── Scheduling state (in-memory per conversation) ───────────────────────────
const pendingSlots = new Map<string, { isoTime: string; label: string }[]>();
...
export async function processIncomingMessage(
  ...
  // 5.6. Inject scheduling context if available
  const schedulingContext = buildSchedulingContext(conversation.id);

  // 6. Call OpenAI
  const messages: OpenAI.Chat.ChatCompletionMessageParam[] = [
    { role: "system", content: SYSTEM_PROMPT + audioPromptSection },
    ...chatHistory,
    { role: "user", content: schedulingContext + contextPrefix + messageText },
  ];
  ...
  // Handle CHECK_AVAILABILITY action — fetch Cal.com slots
  if (agentResponse.actions?.includes("CHECK_AVAILABILITY")) {
    console.log(`[WA Agent] Checking Cal.com availability for conversation ${conversation.id}`);
    const availability = await getAvailableSlots();
    if (availability.success && availability.slots.length > 0) {
      pendingSlots.set(conversation.id, availability.slots);
      console.log(`[WA Agent] Stored ${availability.slots.length} pending slots for ${conversation.id}`);
    } else {
      console.warn(`[WA Agent] No slots available or error:`, availability.error);
      pendingSlots.delete(conversation.id);
    }
  }

  // Handle BOOK_MEETING action — create booking on Cal.com
  if (agentResponse.actions?.includes("BOOK_MEETING") && agentResponse.booking_data) {
    const { slot_index, email, name } = agentResponse.booking_data;
    const slots = pendingSlots.get(conversation.id);

    if (slots && slots[slot_index]) {
      const selectedSlot = slots[slot_index];
      console.log(`[WA Agent] Booking slot ${slot_index}: ${selectedSlot.label} for ${name} (${email})`);
      ...
      if (bookingResult.success) {
        // Format confirmation and append to message
        const confirmation = formatBookingConfirmation(bookingResult, name);
        finalMessage += `\n\n${confirmation}`;
        finalMessage += `\n\n📞 Antes de la reunión es posible que Adrian te llame para hacerte un par de preguntas así aprovechan mejor el tiempo de la videollamada.`;

        // Notify Adrian
        const adminNotification = `📅 *Nueva reunión agendada*\n\n👤 ${name}\n📧 ${email}\n📱 +${cleanPhone}\n🕐 ${selectedSlot.label}\n\n${bookingResult.meetingUrl ? `🔗 ${bookingResult.meetingUrl}` : ""}`;
        await sendWhatsAppMessage(ADRIAN_PHONE, adminNotification);
        ...
        finalMessage += `\n\nHubo un inconveniente al agendar. Intentemos de nuevo en un momento.`;
      }
    } else {
      console.error(`[WA Agent] Invalid slot_index ${slot_index} or no pending slots`);
      finalMessage += `\n\nPerdón, hubo un error con el horario. ¿Podés elegir de nuevo?`;
    }
  }
...
// ─── Build scheduling context for the LLM ────────────────────────────────────
/**
 * If the agent previously requested CHECK_AVAILABILITY and we stored slots,
 * inject them into the context so the LLM knows what options are available.
 */
function buildSchedulingContext(conversationId: string): string {
  const slots = pendingSlots.get(conversationId);
  ...
  return `[SLOTS DISPONIBLES en tu agenda de Cal.com:\n${slotList}\n\nUsá estos horarios EXACTOS para presentarle las opciones al usuario. Cuando el usuario elija, pedile el email y luego incluí BOOK_MEETING en actions con el slot_index (0-based) correcto.]\n\n`;
}
