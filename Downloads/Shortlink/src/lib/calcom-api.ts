...
function calHeaders(endpoint: "slots" | "bookings" = "slots"): Record<string, string> {
  // Bookings endpoint requires a newer API version than slots
  const version = endpoint === "bookings" ? "2024-08-13" : CAL_API_VERSION;
  return {
...
export async function createBooking(
  ...
  try {
    // Convert start time to UTC ISO format (remove timezone offset)
    ...
    console.log(
      `[Cal.com] Creating booking: ${startUTC} for ${attendeeName} (${attendeeEmail})`
    );

    const response = await fetch(`${CAL_BASE_URL}/bookings`, {
      method: "POST",
      headers: calHeaders("bookings"),
